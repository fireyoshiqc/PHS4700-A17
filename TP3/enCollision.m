function [estEnCollision autoEnCollision coinEnCollision surfaceEnCollision] = enCollision(posCentreAutoA, orientationXYAutoA, posCentreAutoB, orientationXYAutoB)
  ...
  for row=1:size(coinsASurfaceB, 1)
    ...
    if all(coinsASurfaceB(row,:) < 0)
      ...
      coinEnCollision = row; # le numéro du coin de auto A
  ...
  for row=1:size(coinsBSurfaceA, 1)
    ...
    if all(coinsBSurfaceA(row,:) < 0)
      ...
      coinEnCollision = row; # le numéro du coin de auto B
      # Determiner la surface de collision
